...
class Settings:
    ...
    def data_dir(self) -> Path:
        ...
        return (
            ...
            if override
            else Path(user_data_dir("libbyctl", appauthor=False))
        )
    ...
    def load(cls) -> Settings:
        ...
        settings.preferred_formats = list(
            planner.get("preferred_formats", settings.preferred_formats)
        )
        ...
        settings.thunder_base_url = str(
            providers.get("thunder_base_url", settings.thunder_base_url)
        )
        settings.thunder_client_id = str(
            providers.get("thunder_client_id", settings.thunder_client_id)
        )
        _ = general
        return settings
    ...
    def save(self) -> None:
        ...
        content = (
            ...
            f"avoid_duplicate_holds = {str(self.avoid_duplicate_holds).lower()}\n\n"
            f"[network]\nmax_concurrency = {self.max_concurrency}\n"
            f"timeout_seconds = {self.timeout_seconds}\n\n"
            f"[providers]\nlibby_base_url = \"{self.libby_base_url}\"\n"
            f"thunder_base_url = \"{self.thunder_base_url}\"\n"
            f"thunder_client_id = \"{self.thunder_client_id}\"\n"
        )
